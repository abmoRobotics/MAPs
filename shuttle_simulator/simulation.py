...
class MultiShuttleSimulator:
    def __init__(self, num_shuttles, grid_size):
        self.shuttles = [PlanarMotor(position=[i * 2., 0.], idx=i, grid_size=grid_size) for i in range(num_shuttles)]
        self.initial_positions = [copy.deepcopy(shuttle.position) for shuttle in self.shuttles]
    ...
    def _get_other_shuttles(self, index):
        return [shuttle for shuttle in self.shuttles if shuttle.idx != index]

    def get_other_shuttle_positions(self, index):
        return [shuttle.position for shuttle in self._get_other_shuttles(index)]

    def collision_detection(self):
        for i in range(len(self.shuttles)):
            for j in range(i + 1, len(self.shuttles)):
                # If circles overlap, return True
                if (self.shuttles[i].position[0] - self.shuttles[j].position[0]) ** 2 + (self.shuttles[i].position[1] - self.shuttles[j].position[1]) ** 2 < 1:
                    # Reset both shuttles to their initial positions
                    self.shuttles[i].position = copy.deepcopy(self.initial_positions[i])
                    print(self.initial_positions)
                    self.shuttles[j].position = copy.deepcopy(self.initial_positions[j])  
